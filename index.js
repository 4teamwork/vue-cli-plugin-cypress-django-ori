module.exports = (api, options) => {
  ...
  api.registerCommand('test:e2e:django', {
    description: 'run e2e tests with Cypress and django',
    usage: 'vue-cli-service test:e2e:django [options]',
    options: {
    ...
    const cwd = join(api.service.context, '..');

    const env = Object.assign({}, process.env);
    env.DJANGO_ENV = 'TestE2E';

    const backend = execa(
      'bin/manage.py',
      ['runserver', process.env.BACKEND_PORT],
      { env, cwd, detached: true }
    );
